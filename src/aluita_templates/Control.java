...
public class Control {
    ...
    public static void unirArquivos(String folder, String filtro) throws Exception {
        File folderFile = new File(folder);
        if (!folderFile.exists()) {
            throw new Exception("A pasta '" + folder + "' não existe!");
        }
    ...
    public class pagamentosFornecedor extends Executavel {
        ...
        public void run() {
            ...
            if (folder.exists()) {
                ...
                String comparation = ComparacaoTemplates.getComparacaoString("Pasta PFOR",
    ...
    /**
     * 
     * With Aluita_tenplates.ini.get("folders").fetch("retorno"), access subfolder
     * by name in constructor.
    ...
    public class pastaRetorno extends Executavel {
        ...
        public void run() {
            ...
            // for each lcto in importation, if filter is filter of string of
            // lcto.getHistorico(), add to list to delete
            for (LctoTemplate lcto : imp.getLctos()) {
    ...
    // function contasAPagar extending Executavel receiving imp and templateSection
    // to call contasAPagar
    public class contasAPagar extends Executavel {
        ...
        public void run() {
                ...
                for (LctoTemplate conta : contas_lctos) {
                    ...
                    if (lcto.getData().equals(conta.getData())) {
                        ...
                        if (lcto.getValor().compareTo(conta.getValor()) == 0) {
                            ...
                            conta.setComplementoHistorico(
                                    conta.getComplementoHistorico() + " " + lcto.getComplementoHistorico());
            ...
            String comparation = ComparacaoTemplates.getComparacaoString(
                    "Contas a pagar: " + config.fetch("contas_a_pagar_coluna").replace(";", " "),
                    "Extrato valor e data igual", lctos_to_add, lctos_to_delete);
            // save comparation on informations if comparation is not empty
            if (!comparation.isEmpty()) {
                Aluita_Templates.informations.get(imp.getNome()).append("\n").append(comparation);
            }
    ...
    // class showInformation extending Executavel receiving importation, get
    // Aluita_Templates.information(importation nome) and throw new Warning
